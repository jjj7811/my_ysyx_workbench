...
InstBuf iringbuf[MAX_INST_TO_PRINT];

int iringbuf_p = 0;  // 应全局变量

void trace_inst(word_t pc, uint32_t inst) {
  iringbuf[iringbuf_p].pc = pc;
  iringbuf[iringbuf_p].inst = inst;
  iringbuf_p = (iringbuf_p + 1) % MAX_INST_TO_PRINT;
}

void print_iringbuf() {
  char buf[128];
  printf("--------printf_iringbuf--------\n\n");
  // Log("---printf_iringbuf---\n");
  for (int i = 0; i < MAX_INST_TO_PRINT; i++) {
    if (i == iringbuf_p-1) printf("----->");
    printf("\tpc = %08x, inst = %08x", iringbuf[i].pc, iringbuf[i].inst);
    // Log("\tpc = %08x, inst = %08x\n", iringbuf[i].pc, iringbuf[i].inst);
  void disassemble(char *str, int size, uint64_t pc, uint8_t *code, int nbyte);
  IFDEF(CONFIG_ITRACE,disassemble(buf, 128,
              iringbuf[i].pc,
...
static void exec_once(Decode *s, vaddr_t pc) {
  s->pc = pc;
  s->snpc = pc;
  // 这里获取指令，写入s->isa.inst.val，也就是说我需要从这里取出指令写入ringbuf。
  isa_exec_once(s);
  IFDEF(CONFIG_ITRACE,trace_inst(s->pc, s->isa.inst.val));  // 写入iringbuf
  cpu.pc = s->dnpc;
